...
## 版型規範
1. 後台統一使用左側主導覽 + 頂部全域操作列
2. 列表頁使用 `Header / Filter / Table / Batch Actions / Pagination`
3. 詳情頁使用 `Summary / Tabs / Activity / Side Actions`
4. 高密度頁面支援使用者自訂欄位顯示

## 風險操作頁面
1. `/admin/deployments/rollback`
2. `/admin/dr/restore`
3. `/admin/settings/api-keys`
4. `/admin/users/:id`

## 頁面交付標準
1. 每頁需定義空狀態、錯誤狀態、權限不足狀態
2. 每頁需有對應功能模組與權限來源
3. 每頁需明確區分主操作與次操作
